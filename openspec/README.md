...
| FR-04 会话恢复       | `p1-relay-session-lifecycle`, `p1-companion-session-management`, `p0-openclaw-bridge`, `p2-android-workspace-settings`, `p2-android-session-detail`, `persistent-interactive-sessions`, `p3-session-detail-hardening`, `p1-session-catalog-sync`                                                                                                |
| FR-05 多会话并发     | `p1-relay-session-lifecycle`, `p1-reconnect-and-catchup`, `p2-android-session-list`, `p3-error-ux-and-cleanup`, `p1-session-catalog-sync`                                                                                                                                                                                                       |
| FR-06 流式输出与渲染 | `p0-android-prototype`, `p2-android-session-detail`, `p3-theme-and-animations`, `persistent-interactive-sessions`, `p3-session-detail-hardening`, `p3-message-rendering-polish`, `p3-message-copy`, `p3-tool-call-rich-display`, `p3-ask-user-question-fix`, `p3-longpress-menu-fix`, `p3-initial-user-message-echo`, `p3-detail-display-fixes` |
...
| FR-10 审批保留       | `p3-approval-path-reserved`, `p3-mobile-skill-interaction`, `p3-dynamic-slash-catalog`                                                                                                                                                                                                                                                          |
...
- `p1-session-catalog-sync` — 主动 session 目录同步：周期对账、本地活跃时间刷新、Android 首页自动刷新
...
- `p3-dynamic-slash-catalog` — runtime slash catalog: companion scans installed commands/skills and reconciles them with Claude runtime `init.slash_commands`, relay exposes host catalog API, Android detail uses dynamic slash sheet
...
- `p3-detail-display-fixes` — 详情页三项显示 bug 修复：Skill 内容泄露抑制、工具结果自动折叠、上下文用量 cache tokens 缺失

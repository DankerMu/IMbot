...
**断言**:
- 截图中 agent 消息为浅色 assistant 气泡（非页面底色直出）
- provider 头像 badge 可见（圆形彩色背景 + 首字母缩写）
- 消息文本与头像之间有间距（~12dp）
- assistant 气泡左对齐，圆角与用户气泡镜像
- 时间戳在消息下方，小字号灰色
...
2. dump UI → 定位顶栏右上角状态指示器
3. 截图 "polish-v2-status-minimal"

**断言**:
- 顶栏右上角显示状态指示器，并随 session 状态在 `running / idle` 之间刷新
- 状态文字左侧有状态点，颜色与状态语义对应（running=绿色, idle=蓝色）
- 消息时间线中不再插入独立的 `运行中 / 空闲` 状态文本
- 会话结束后顶部状态仍以顶栏指示器为准，不依赖消息流中的状态项
...
- 点击 FAB → 自动滚动到底部
- 在用户未主动上滑的情况下，长回复应保持自动贴底，不能因单条消息过长而错误弹出 FAB
